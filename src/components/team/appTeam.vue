  ...
  <v-container class="pa-0 mb-5">
    <v-layout wrap align-start justify-start row fill-height class="mt-0 mb-0">
      <v-flex xs12 class="mb-3">
        <p class="google-font" style="font-size:130%;">Our App Developers</p>
        <v-divider></v-divider>
      </v-flex>
            ...
            <v-list-tile :key="item.name" avatar>
              ...
              <v-list-tile-content>
                ...
                <v-list-tile-sub-title>
                  <span class="google-font">{{item.designation}}</span>
                </v-list-tile-sub-title>
                <v-list-tile-sub-title>
                  <v-btn
                    class="mt-0 mx-0"
...
<script>
import TeamDetails from "@/assets/data/appTeam.json";
export default {
  data() {
    return {
  ...
  methods: {
    getImgUrl(pic) {
      if (pic.length > 0) {
        return require("@/assets/img/team/app-team/" + pic);
      } else {
